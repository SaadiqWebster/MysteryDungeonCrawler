...
class Minimap:
    ...
    def draw(self):
        surf = pygame.Surface(((self.floormanager.floor.floor_width * self.unit_size) + 4, (self.floormanager.floor.floor_height * self.unit_size) + 4))
        ...
        for i in range(self.floormanager.floor.floor_width):
            for j in range(self.floormanager.floor.floor_height):
                ...
                if self.floormanager.get_visibility_map(map_cor) > 0:
                    pin_radius = math.ceil(self.unit_size / 2)
                    ...
                    if map_cor == self.floormanager.stairs_cor:
                        pygame.draw.rect(cell, self.stairs_color, pygame.Rect(0, 0, self.unit_size, self.unit_size), 1)
    ...
    def draw_wall_cell(self, map_cor):
        ...
        for i in range(len(dir_x)):
            neighbor_cor = [map_cor[0] + dir_x[i], map_cor[1] + dir_y[i]]
            if self.floormanager.floor.isinbound(neighbor_cor) and self.floormanager.floor.get_floor_map(neighbor_cor) == 1:
                if i == 0:
                    pygame.draw.line(cell, (255, 255, 255), (0, 0), (self.unit_size-1, 0))
                if i == 1:
                    cell.set_at((self.unit_size-1, 0), (255, 255, 255))
